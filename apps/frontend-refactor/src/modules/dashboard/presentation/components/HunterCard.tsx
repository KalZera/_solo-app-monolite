...
export function HunterCard({ summary }: { summary: DashboardSummary }) {
  ...
  const remaining = Math.max(0, summary.xpToNext - summary.xp)
    ...
    <Panel className="gap-4">
      ...
      <View className="gap-1.5">
        <View className="flex-row justify-between">
          <Text className="text-xs text-content-muted">{t('dashboard.experience')}</Text>
          <Text className="text-xs text-content-muted">
            {summary.xp} / {summary.xpToNext}
          </Text>
        </View>
        <ProgressBar value={summary.xp} max={summary.xpToNext} tone="legendary" />
        <Text className="text-[11px] text-content-muted">
          {t('dashboard.toNextLevel', { xp: remaining })}
        </Text>
      </View>
    </Panel>
  )
