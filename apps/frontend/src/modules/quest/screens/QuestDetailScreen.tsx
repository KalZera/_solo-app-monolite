...
export function QuestDetailScreen({ questId }: QuestDetailScreenProps) {
  ...
  const isInactiveStatus = quest
    ? INACTIVE_STATUSES.includes(quest.status)
    : false;
  const isMainQuest = quest?.type === "main";
  const objectivesRatio = quest
    ? calculateObjectivesCompletionRatio(quest.objectives)
    : 0;
  const objectivesThresholdMet =
    !isMainQuest || objectivesRatio > MAIN_QUEST_COMPLETION_THRESHOLD;
  const completedObjectivesCount =
    quest?.objectives.filter((objective) => objective.completed).length ?? 0;
          ...
          <SystemPanel gap="$4">
            ...
            {quest.expiresAt && (
              <Text color="$soloTextMuted" fontSize="$2">
                {t("quest.detail.deadline", {
                  date: new Date(quest.expiresAt).toLocaleString(),
                })}
              </Text>
            )}
                ...
                {quest.objectives.map((objective) => (
                  <ObjectiveRow
                    ...
                    isCompleting={
                      ...
                      completeObjective.variables?.objectiveId === objective.id
                    }
            ...
            {isInactiveStatus ? (
              ...
              >
                {t("quest.detail.statusLabel", {
                  status: t(`quest.statuses.${quest.status}`, {
                    defaultValue: quest.status,
                  }),
                })}
              </Text>
              ...
              >
                {completeQuest.isPending
                  ? t("quest.detail.completing")
                  : t("quest.detail.complete")}
              </SystemButton>
            )}
          </SystemPanel>
        )}
