...
import { isCharacterNotFound, useCharacterProfile } from '@/modules/character/api/useCharacterProfile'
...
export function HomeScreen() {
  const { data: character, isPending: isCharacterPending, isError, error } = useCharacterProfile()
  const { data: quests, isPending: isQuestsPending } = useDailyQuests({ enabled: Boolean(character) })
  ...
  if (isError) {
    return (
      <YStack flex={1} justifyContent="center" alignItems="center" backgroundColor="$soloBg" padding="$5">
