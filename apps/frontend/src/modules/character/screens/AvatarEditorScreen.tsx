...
import { Camera, Check, ChevronLeft, Images, User, ZoomIn, ZoomOut } from '@tamagui/lucide-icons-2'
...
function clampTranslate(candidate: Offset, userScale: number, image: PickedImage): Offset {
...
export function AvatarEditorScreen() {
  ...
  function applyPickedAsset(result: ImagePicker.ImagePickerResult) {
    ...
    if (!asset?.width || !asset.height) {
      showError(t('character.avatarEditor.toastErrorTitle'), t('character.avatarEditor.pickError'))
      return
    }
  ...
  async function pickFromLibrary() {
    ...
    if (!permission.granted) {
      showError(t('character.avatarEditor.toastErrorTitle'), t('character.avatarEditor.libraryPermissionDenied'))
      return
    }

    applyPickedAsset(await ImagePicker.launchImageLibraryAsync({ mediaTypes: ['images'], quality: 1 }))
  }

  async function takePhoto() {
    const permission = await ImagePicker.requestCameraPermissionsAsync()
    if (!permission.granted) {
      showError(t('character.avatarEditor.toastErrorTitle'), t('character.avatarEditor.cameraPermissionDenied'))
      return
    }

    applyPickedAsset(await ImagePicker.launchCameraAsync({ mediaTypes: ['images'], quality: 1 }))
  }

  function adjustZoom(delta: number) {
    if (!image) return
    const nextScale = Math.min(MAX_SCALE, Math.max(MIN_SCALE, scaleRef.current + delta))
    setScale(nextScale)
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      const originX = image.width / 2 - VIEWPORT_SIZE / (2 * displayScale) - translate.x / displayScale
      const originY = image.height / 2 - VIEWPORT_SIZE / (2 * displayScale) - translate.y / displayScale
      ...
      showError(t('character.avatarEditor.toastErrorTitle'), t('character.avatarEditor.cropError'))
    } finally {
      setIsCropping(false)
    }
  }
  ...
  return (
    <YStack flex={1} backgroundColor="$soloBg" padding="$4" paddingTop="$7" gap="$5" alignItems="center">
      ...
      <SystemPanel width="100%" maxWidth={420} alignItems="center" gap="$4">
        ...
        >
          {image ? (
            <RNImage source={{ uri: image.uri }} style={previewStyle} resizeMode="cover" />
          ) : (
        ...
        <Text color="$soloTextMuted" fontSize="$2" textAlign="center">
          {image ? t('character.avatarEditor.cropHint') : t('character.avatarEditor.editHint')}
        </Text>
        ...
        <XStack gap="$3" width="100%">
          ...
          >
            {image ? t('character.avatarEditor.changePhoto') : t('character.avatarEditor.chooseFromLibrary')}
          </Button>
          <Button
            flex={1}
            chromeless
        ...
        {image && (
          ...
          >
            {isBusy ? t('character.avatarEditor.confirming') : t('character.avatarEditor.confirm')}
          </SystemButton>
        )}
      </SystemPanel>
    </YStack>
