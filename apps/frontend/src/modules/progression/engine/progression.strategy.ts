...
// accumulated XP does each level cost". Kept behind an interface so the
// ProgressionEngine stays closed for modification but open for extension
// (Strategy Pattern): seasons, prestige curves, double-XP events or entirely
// different pacing can be introduced by providing another strategy.
//
// This is a faithful port of the backend engine so the client renders exactly
// the same curve the server awards XP on — no divergent client-only math.

// Tunable parameters of the default continuous curve. Centralised here so there
// are no magic numbers in the math, and a curve can be reshaped purely through
// configuration.
export interface ProgressionConfig {
  // Accumulated XP required to reach level 1, and the value the curve is anchored
  // to at level 1 (see ContinuousCurveStrategy).
  baseXp: number
...
//   - calculateLevel(12500) === 12 and getProgress(750) yields
//     nextLevelXp 1161 / progress 37.82%.
...
//
// This shape algebraically guarantees the two hard business rules for ANY
// exponent/multiplier:
