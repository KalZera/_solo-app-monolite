...
import { ProgressionEngine, type LevelProgress } from './progression.engine'
import {
...
describe('ProgressionEngine (frontend)', () => {
  ...
  describe('getXpIntoCurrentLevel / getRemainingXp', () => {
    ...
    it('is 0 / a full span on a fresh character', () => {
  ...
  describe('getProgress', () => {
    ...
    it('describes the level-12 mock character coherently', () => {
      const snapshot = engine.getProgress(12450)
      expect(snapshot.level).toBe(12)
      expect(snapshot.attributePointsAvailable).toBe(60)
      expect(snapshot.xpIntoCurrentLevel + snapshot.xpRemaining).toBe(
        snapshot.nextLevelXp - snapshot.currentLevelXp,
      )
    })

    it('keeps every derived field internally consistent', () => {
      for (const totalXp of [0, 250, 500, 750, 1161, 5000, 12450, 12500, 250_000]) {
