...
// it's flipped to active = false. Templates with a future deadlineDate — or none at all — are
// left untouched; this job only ever turns quests off, never back on.
export class DeactivateExpiredQuestsUseCase {
  ...
  async execute (now: Date = new Date()): Promise<Quest[]> {
    ...
    const expired = activeQuests.filter((quest) => quest.deadlineDate !== null && quest.deadlineDate.getTime() < now.getTime())
